...
public class BasicController {
	...
	public String calculator(@PathVariable int won, Model model) {
		final double YEN_RATE = 9.91;
		double result = (won / YEN_RATE);
		model.addAttribute("result",result);
		model.addAttribute("won",won);
		return "rate";
	}
	...
	public String getUserInfo(UserVO userVO ,Model model) {
		// 변수명은 userVO로 저장했으나, 실제로는 내부 멤버변수의 이름으로 데이터를 받습니다.
