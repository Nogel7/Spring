...
public class BoardController {
	...
	@Autowired
	private BoardService service;//(실제 주입되는 요소는 ServiceImpl이므로 호출지점도 ServiceImpl이다.)
	...
	// @RequesParam(name="사용할변수명",defaultValue="지정하고싶은기본값") 변수 왼쪽에 저렇게 붙여주면 처리완료.
	...
	public String boardList(SearchCriteria cri, Model model) {
		//if(pageNum == null) {
		//	pageNum = 1L;// Long형은 숫자 뒤에 L을 붙어야 대입됩니다.
		//}
	...
	// 글 하나만 조회할 수 았는 디테일 페이지인  boardDetail.jsp로 연결되는
	// / boardDetail 주소를 get방식으로 선언해주세요.
	// 주소 뒤에 ?bno=번호 형식으로 적힌 번호 글만 조회합니다.
	// 숙제: @PathVariable 적용 방식으로 바꿔보세요. boardList에서 제목클릭시 넘어오게해주세요.
	...
	// 입력받은 BoardVO를 토대로 mapper쪽의 insert메서드를 실행해 주시고
	...
	// /boardUpdateForm 를 post방식으로 접속하는 form연결 메서드를 만들겠습니다.
	// / update로직은 이미 데이터가 입력이 되어 있어야 합니다.
	// 따라서 내가 수정하고자 하는 글의 정보를 VO로 받아온다옴
